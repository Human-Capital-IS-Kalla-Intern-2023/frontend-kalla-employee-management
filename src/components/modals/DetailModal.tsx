            ...
            {Object.keys(data).map((key) => (
              <div className="my-2" key={key}>
                ...
                {Array.isArray(data[key])
                  ? data[key].map((arrayData: any, index: any) => (
                      <div key={index}>
                        <strong>ID:</strong> {arrayData.id} <br />
                        <strong>Location Name:</strong>{' '}
                        {arrayData.location_name} <br />
                        <strong>Created At:</strong> {arrayData.created_at}{' '}
                        <br />
                        <strong>Updated At:</strong> {arrayData.updated_at}{' '}
                        <br />
                        <strong>Deleted At:</strong> {arrayData.deleted_at}{' '}
                        <br />
                      </div>
                    ))
                  : data[key]}
